<script lang="ts">
	...
	let navMenuWrapper: any;

	onMount(() => {
		const handleClickOutside = (event: MouseEvent) => {
			if (container && navMenuWrapper) {
				if (!container.contains(event.target as Node) && !navMenuWrapper.contains(event.target as Node)) {
					goto('/');
				}
			}
		};
